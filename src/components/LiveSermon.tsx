...
export function LiveSermon({ variant = 'full' }: { variant?: 'compact' | 'full' }) {
  ...
  // The "uploads" playlist (past live-stream videos, most of which YouTube
  // blocks from embedding) is exempt from the search/empty-drop logic since
  // it renders a static "watch on YouTube" link instead of a video list —
  // it stays visible whenever the pill filter allows it, search or not.
    ...
    return data.playlists
      ...
      .map((p) => ({
        ...
        videos:
          p.kind === 'uploads'
            ? p.videos
            : p.videos.filter((v) => (q ? v.title.toLowerCase().includes(q) : true)),
      }))
      .filter((p) => p.kind === 'uploads' || p.videos.length > 0)
  }, [data.playlists, activePlaylist, query])
            ...
            visiblePlaylists.map((playlist) => (
              <div key={playlist.id}>
                ...
                {playlist.kind === 'uploads' ? (
                  // Past live-stream videos almost always carry a YouTube
                  // Content ID claim that disables embedding — a
                  // restriction on YouTube's side, not fixable from here —
                  // so this section links out instead of showing broken
                  // embeds. Curated playlists below aren't affected.
                  <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-300 p-6 text-center dark:border-night-700">
                    <p className="max-w-md text-sm text-slate-600 dark:text-night-300">{t.sermons.pastLivesNote}</p>
                    <a
                      ...
                      >
                        ...
                        <div className="flex flex-1 flex-col gap-1 p-3">
                          ...
                          <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-2 text-xs dark:border-night-800">
                            <span className="text-slate-500 dark:text-night-400">
